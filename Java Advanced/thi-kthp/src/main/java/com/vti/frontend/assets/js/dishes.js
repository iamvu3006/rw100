...
async function loadCategoriesIntoFilters(){
  ...
  categories.forEach(c=>{
    filterSel.insertAdjacentHTML("beforeend", `<option value="${c.id}">${escapeHtml(c.name)}</option>`);
...
async function loadDishes(){
  ...
  try{
    ...
    qs("#prevPg")?.addEventListener("click", ()=>{ currentPage--; loadDishes(); });
    qs("#nextPg")?.addEventListener("click", ()=>{ currentPage++; loadDishes(); });

    qsa("[data-edit]").forEach(b=> b.addEventListener("click", ()=> openEdit(b.dataset.edit)));
    qsa("[data-del]").forEach(b=> b.addEventListener("click", ()=> deleteDish(b.dataset.del)));
  }catch(err){
    body.innerHTML = "";
    pagination.style.display = "none";
...
qs("#dishForm").addEventListener("submit", async (e)=>{
  ...
  if(!payload.name || !payload.categoryId || isNaN(payload.price)){
    qs("#formError").textContent = "Vui lòng nhập tên món, giá và chọn danh mục.";
    return;
  }
...
async function deleteDish(id){
  const ok = await confirmDialog({ title:"Xóa món ăn", message:"Món ăn sẽ bị xóa khỏi thực đơn. Tiếp tục?", okText:"Xóa" });
  if(!ok) return;
  try{ await api.del(`/dishes/${id}`); toast("Đã xóa món ăn","success"); loadDishes(); }
  catch(err){ toast(err.message,"error"); }
}

[qs("#categoryFilter"), qs("#statusFilter")].forEach(el=> el.addEventListener("change", ()=>{ currentPage=0; loadDishes(); }));
let searchDebounce;
qs("#searchInput").addEventListener("input", ()=>{
  clearTimeout(searchDebounce);
  searchDebounce = setTimeout(()=>{ currentPage=0; loadDishes(); }, 350);
});
