...
+ Với interface thì viết hoa chữ cái đầu tiên của mỗi từ,
  bắt đầu bằng chữ "I", không có dấu cách (ví dụ: IPosition, IAccount, IDepartment,...).
...
6. Đây là cấu trúc database của tôi:
   CREATE DATABASE rw100_final_exam_2;
   USE rw100_final_exam_2;
   CREATE TABLE `User` (
   `id` INT AUTO_INCREMENT PRIMARY KEY,
   `FullName` VARCHAR(255) NOT NULL,
   `Email` VARCHAR(255) NOT NULL UNIQUE,
   `Password` VARCHAR(255) NOT NULL,
   `Role` VARCHAR(50) NOT NULL, -- Dùng để phân biệt 'Admin' hoặc 'Employee'

   -- Thuộc tính riêng (Bắt buộc để NULL)
   `ExpInYear` INT NULL,        -- Chỉ dành cho Admin
   `ProSkill` VARCHAR(255) NULL -- Chỉ dành cho Employee
   );