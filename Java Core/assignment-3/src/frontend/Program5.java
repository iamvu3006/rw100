...
public class Program5 {
    public static void main(String[] args) {
        ...
        for (Department department : sorted) {
            System.out.println("Câu 6: Danh sách phòng ban theo thứ tự tăng dần: " + department.name);
        }

        Department[] sortedManual = exercise.question7SortDepartmentsByNameManual(sorting);
        for (Department department : sortedManual) {
            System.out.println("Câu 7: Danh sách phòng ban sau khi sắp xếp theo tên: " + department.name);
        }
    }
}
